...
def Cswap(qc: QuantumCircuit, control: int, target1: int, target2: int):
    m_target_1 = qc.qubits[target1].measure()
    m_target_2 = qc.qubits[target2].measure()

    if qc.qubits[control].measure() == 1 and m_target_1 != m_target_2:
        qc.qubits[target1].measurement = m_target_2
        qc.qubits[target2].measurement = m_target_1
...
for i in range(100000):
    ...
    qc.cnot(7, 8)
    qc.cnot(8, 9)
    measure = tuple(qc.measure_all())
    if not probability.get(measure, False):
...
for key, value in probability.items():
    ...
    for i in key:
        num = 2 * num + i
    table[bin(num)[2:]] = value
    value_total += value

for key, value in table.items():
    print(f"basis_state: {key}, probability: {(value * 100) / value_total} +- 0.5%")
